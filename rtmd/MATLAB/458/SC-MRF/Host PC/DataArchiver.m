...
csvFilename = ['results\DAQ_' datestr(now,'yyyy-mm-dd--HH-MM-SS') '.csv'];
...
while (true) 
    ...
    % If SCRAMNet Sim Bit is disabled, wait
    while (scr.readFloat(SIM_BIT) == 0)
        pause(0.01);
    end     
    
    % Pause for a certain delay
    pause(delay);
end