...
while (read (STDIN ,$LINE, 4096*10) && $bRead < $len )
{
	$bRead += length $LINE;
...
while(($key,$value) = each %vars)
{
	...
	{	
		...
		if(defined $fh)
		{
    		...
    		$tmp_filename = substr $tmp_filename, 1 + rindex $tmp_filename, '\\'; 
...
my $url = $pathname . "UniqueName=$uniquename&sid=$sessionid&Add=1" ;
