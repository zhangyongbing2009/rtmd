function varargout = HybridCR(varargin)
% HYBRIDCR M-file for HybridCR.fig
%      HYBRIDCR, by itself, creates a new HYBRIDCR or raises the existing
%      singleton*.
%
%      H = HYBRIDCR returns the handle to a new HYBRIDCR or the handle to
%      the existing singleton*.
%
%      HYBRIDCR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in HYBRIDCR.M with the given input arguments.
%
%      HYBRIDCR('Property','Value',...) creates a new HYBRIDCR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before PredTarget_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to HybridCR_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help HybridCR

% Last Modified by GUIDE v2.5 02-Jul-2008 09:34:42

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @HybridCR_OpeningFcn, ...
                   'gui_OutputFcn',  @HybridCR_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before HybridCR is made visible.
function HybridCR_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to HybridCR (see VARARGIN)

% Choose default command line output for HybridCR
...
% UIWAIT makes HybridCR wait for user response (see UIRESUME)
...
set(handles.commandsleft,'String',num2str(handles.Vars.steps-1))
...
function varargout = HybridCR_OutputFcn(hObject, eventdata, handles) 
...
if (convergeradiobutton)     
  ...
  while (nonstop) 
    ...
    % Get commands from hybrid PSD algorithm
    handles.Vars.uold = handles.Vars.u;            % Store previous predictor displacement
    handles.Vars.u = handles.Vars.u + handles.Vars.dt*handles.Vars.ud + handles.Vars.beta2*handles.Vars.dt^2*handles.Vars.udd; 
    handles.Vars.ud = handles.Vars.ud + handles.Vars.beta1*handles.Vars.dt*handles.Vars.udd;    
    set(handles.target1,'String',num2str(handles.Vars.u(1)));
    set(handles.target2,'String',num2str(handles.Vars.u(2)));
    set(handles.target3,'String',num2str(handles.Vars.u(3)));
    set(handles.target4,'String',num2str(handles.Vars.u(4)));
    handles.Vars.results_commands(handles.Vars.HybridStepCount,:) = handles.Vars.u';
    ...
    %Update Spring models and get restoring force
    updateHybridModelExp
    
    % Calculate acceleration vector for hybrid algorithm
    getNextStepAcceleration     
